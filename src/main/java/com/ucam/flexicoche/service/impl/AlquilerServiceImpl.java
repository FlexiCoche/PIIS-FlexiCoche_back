...
public class AlquilerServiceImpl implements AlquilerService {
	...
	public List<AlquilerDTO> findByCorreo(String correo) {
		...
		if (usuarioOpt.isPresent()) {
			...
			if (usuario.getRoles().contains("ADMIN")) {
				...
				alquileres = alquilerRepository.findByUsuario(correo);
			}
		}
	...
	public void registrarAlquiler(String correo, RegistrarAlquilerDTO alquiler) throws Exception {
		...
		if (usuarioOpt.isPresent()) {
			...
			if (alquileres.stream().anyMatch(
					x -> !x.getEstado().equals(Estado.DENEGADO) && ((x.getFechaInicio().after(alquiler.getFechaInicio())
	...
	public void pagarAlquiler(Long id) {
		...
		if (!alquiler.getEstado().equals(Estado.A_PAGAR)) {
			throw new IllegalStateException(
					"Este alquiler no puede ser pagado en su estado actual: " + alquiler.getEstado());
		}
		...
		try {
			...
			long dias = ChronoUnit.DAYS.between(alquiler.getFechaInicio().toLocalDateTime(),
					alquiler.getFechaFin().toLocalDateTime());
	...
	@Override
	public void cambiarEstado(String correo, EstadoDTO estado) throws Exception {

		if (estado.getIdAlquiler() == null || estado.getNewEstado() == null) {
			throw new Exception("Faltan datos obligatorios");
		}
		...
		if (usuarioOpt.isPresent()) {
			Usuario usuario = usuarioOpt.get();
			if (!usuario.getRoles().contains("ADMIN")) {
				throw new Exception("El usuario no tiene permisos para cambiar el estado del alquiler");
			}

			Optional<Alquiler> alquilerOpt = alquilerRepository.findById(estado.getIdAlquiler());
			...
			try {
				newEstado = Estado.fromNombre(estado.getNewEstado());
			} catch (IllegalArgumentException e) {
				throw new Exception("Estado no válido");
			}

			Alquiler alquiler = alquilerOpt.get();
			alquiler.setEstado(newEstado);
